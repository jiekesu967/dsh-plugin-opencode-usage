 ...
 *
 * 注册 `sidebar.footer.action` 列表槽位(id="opencode-usage",order 在官方
 * Cordis 面板之后),按钮位于左侧侧边栏 Cordis 按钮与设置按钮之间。点击后
 * 在视口左下角弹出固定悬浮面板,展示订阅套餐的滚动 / 每周 / 每月:
 * 总额度、已用、剩余、使用百分比与重置时间;面板打开期间每 60s 自动刷新。
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ".ou_layer{flex:none;align-items:center;width:100%;height:49px;margin:8px 0 0;display:flex;position:relative}",
      ".ou_footerButtons{align-items:center;width:100%;display:flex}",
      ".ou_badge{width:100%;height:49px;color:var(--dsw-alias-label-primary);cursor:pointer;background:0 0;border:none;border-radius:12px;align-items:center;gap:8px;padding:0 8px 0 6px;font-family:inherit;font-size:14px;display:inline-flex;overflow:hidden}",
      ".ou_badge:hover{background:var(--dsw-alias-interactive-bg-hover-solid)}",
      ".ou_badge[data-open=true]{background:var(--dsw-alias-interactive-bg-hover)}",
      ...
      ".ou_badgeMeta{color:var(--dsw-alias-label-tertiary);font-variant-numeric:tabular-nums;flex:none;margin-left:auto;font-size:12px;line-height:16px}",
      ".ou_layer.ou_rail{width:36px;height:36px;margin:0}",
      ".ou_rail .ou_badge{border-radius:50%;justify-content:center;gap:0;width:36px;height:36px;padding:0}",
      ".ou_rail .ou_footerButtons{flex-direction:column;gap:2px}",
      ".ou_panel{z-index:35;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-base);width:min(392px,calc(100vw - 24px));max-height:min(74vh,640px);box-shadow:var(--dsw-shadow-lv2);--dsh-scrollbar-thumb:var(--dsw-alias-scrollbar-bg-l2);--dsh-scrollbar-thumb-hover:var(--dsw-alias-scrollbar-hover-l2);border-radius:14px;flex-direction:column;display:flex;position:fixed;bottom:128px;left:12px;overflow:hidden;transition:left var(--ds-transition-duration-slow) var(--ds-ease-in-out),width var(--ds-transition-duration-slow) var(--ds-ease-in-out)}",
    ...
    function OpenCodeUsagePanel(props) {
      ...
      var PANEL_MIN_SPACE_RIGHT = 200;
      var VIEWPORT_GAP = 12;
      var SIDEBAR_GAP = 12;

      /** 找到包含本按钮的侧边栏列(AppFrame 的 overflow:hidden 全高列)。 */
      ...
      /** 根据侧边栏实际宽度计算悬浮面板 left / width。 */
      function computePlacement() {
        var viewportWidth = window.innerWidth || document.documentElement.clientWidth || 0;
        var column = findSidebarColumn();
        var fallback = rootRef.current ? rootRef.current.getBoundingClientRect() : null;
        var sidebarRight = column ? column.getBoundingClientRect().right : fallback ? fallback.right : 0;
        var spaceRight = viewportWidth - sidebarRight - SIDEBAR_GAP - VIEWPORT_GAP;

        if (spaceRight >= PANEL_MIN_SPACE_RIGHT) {
          return {
            left: Math.round(sidebarRight + SIDEBAR_GAP),
            width: Math.round(Math.min(PANEL_MAX_WIDTH, spaceRight)),
          };
        }

        var width = Math.min(PANEL_MAX_WIDTH, Math.max(0, viewportWidth - VIEWPORT_GAP * 2));
        var left = Math.max(VIEWPORT_GAP, Math.min(sidebarRight + SIDEBAR_GAP, viewportWidth - width - VIEWPORT_GAP));
      ...
      return React.createElement(
        ...
        {
          ref: rootRef,
          className: wide ? "ou_layer" : "ou_layer ou_rail",
        },
        open
          ? React.createElement(
              "section",
